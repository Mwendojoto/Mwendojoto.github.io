...
function Phrase(phrase, champ, vide, stir, blocs, frags, save) //construit un dictionnaire et un répertoire d'indices
{
	...
	/*while (secondPos>0)
			{
				var first=phrase.indexOf(' '), s=5;
				secondPos=Math.max(phrase.indexOf('\n'),0);
				if (first<secondPos)
					{
						secondPos=first;
						s=3;
				...
				var line=phrase;
				if (secondPos>0)
					line=line.slice(0,secondPos);
				else
					line=line.slice(0,line.length-1);
				if (line.length>3)
					large++;
				var x=new Element(line+" "+secondPos+" "+s,s,[]);
				temporary.push(x);
				phrase=phrase.slice(secondPos+1);	
			}*/
	...
	this.toHTML = function(m)
	{
		...
		var backButton = "<button id=\"backBtn\" style=\"font-size:22px;\" onclick=\"back()\">←</button><br><button id=\"stir\" style=\"font-size:25px;\" onclick=\"reSet()\">×</button>";
